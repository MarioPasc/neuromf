...
Wraps Med3D (3D) and RadImageNet (2.5D) feature extractors with a common
interface. Extracts features from volume HDF5 archives and caches results
to feature HDF5 files for reuse across evaluation runs.
"""
...
class FeatureExtractor:
    ...
    def __init__(
        ...
        if backend == "med3d":
            ...
            self.model = load_med3d_resnet50(weights_path).to(device)
        ...
        with h5py.File(str(volume_h5_path), "r") as vf:
            for i in range(n_samples):
                ...
                if self.backend == "med3d":
                    ...
                    feat = extract_3d_features(vol, self.model)
                else:
                    ...
                    # Average across planes: each is (N_slices, 2048)
                    feat = torch.cat(
                        [
